...
- agentがアクセス可能な保持済みrun、log、source、runtime状態およびread-only probeを自ら先に確認し、それでも有力仮説を区別できないboundaryだけを追加観測または再現する。ユーザーへ依頼するのは、その追加観測または再現をagentが実行または観測できない場合に限る。依頼時は、区別する仮説、再現baseline、比較を伴うなら変更する一要因、取得する観測値、各仮説の反証条件、および所要時間、操作負担、状態変更、再現riskを含むユーザーcostを示す。報告されたbrowser、runtime、
