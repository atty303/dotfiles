...
- 情報漏洩の指摘では、値が与える認証、認可、復号、署名またはなりすまし能力、違反する明示的なconfidentialityまたはprivacy contract、および実際の出力先
- コードまたは検証結果に基づく根拠
- 最小限の修正方針
...
不具合修正では、修正が意図どおり動作すると仮定しても元のfailure oracleがfailし続ける到達可能な反例を探す。特に、修正によって新しく到達可能になったdownstream state、callerとconsumer間で意味が変わる値、retry、re-entry、逆操作、およびtestが直接生成している中間成功状態を確認する。探索は変更によって到達性または意味が変わったproduction経路に限定し、最終契約がleaf functionの返り値やrequest発行で終わる場合に無関係なdownstreamを要求しない。
...
Usernameを含むhome path、hostname、account名、IP、PID、port、device名、repository path、UUID、digestまたはcommit IDは、それ自体ではsecretでもprivacy violationでもない。Authorizedなtask surfaceでの表示をsecurity findingにせず、publicまたは無関係な外部sinkへの不要な識別子はprivacyまたはportabilityとして比例的に扱う。

`spike`では、仮説判定を妨げる欠陥、意図しないdata loss、control domain外へのsecretまたは明示的なconfidential contentの漏洩、現在の出力contractに含まれるpublicまたは無関係な外部sinkへの不要なprivacy-sensitive dataおよび個人・host固有operational identifierの送信・公開、authority逸脱、および一時resourceのcleanup不良だけをactionableな指摘とする。Public sinkが現在の出力contractに含まれない場合のoperational identifier最小化、耐敵対入力、長期運用、互換性、可用性または包括的validationなど、本番化しなければ価値のないhardeningは現在の欠陥にせず、具体的な到達経路を検証できたものだけを重複排除したpromotion checklist候補とする。
