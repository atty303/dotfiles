...
- 必須test、標準checkまたはfailure oracleが予期せず失敗、停止、timeout、または実行ごとに異なる結果を示した場合は、入力変更、代替経路、workaroundまたは根拠のない再試行を行う前に`investigate-problem`を使用する。観測面が不足している場合は、`design-program-observability`へ引き渡してから元の開発へ戻る。実行前に補助的な探索と明示した検査は失敗後に再分類せず、その目的を保持する。
- 現在の差分が原因かと、原因がrepository-controlledかを別に判定する。Product source、test、fixture、test harness、task、build process、workflowまたはrepository内の設定が原因なら、以前から存在した問題や別moduleの問題でも同じrepositoryのtaskを完了する際のblockerとし、既存のdependency、migration、外部状態変更、破壊的変更および権限の承認境界を維持したまま、最も近い原典で原因を除去してから元の開発と検証を再開する。事前に補助的な探索と明示した検査だけは、元taskとの因果関係を除外し、残存riskと未解決事項を報告できる場合に限ってtask全体のblockerにしない。
- 外部service、network、runnerまたは一時的な実行環境を原因とするには、失敗段階を該当boundaryまで特定し、同じrevision、入力および設定による対照を確認し、repository-controlledな不変条件が破られていないことを確認する。外部service名を含むerrorまたは一回の再実行成功だけでは一時的外因とせず、証拠が足りなければ原因未確定として扱う。必須test、標準checkまたはfailure oracleの原因未確定なflakyは、再実行でgreenになっても元taskの完了証拠にしない。一時的外因は恒久成果物または永続化候補に変換しない。
- 恒久修正では、修正前のlog、source、保持済み観測または一要因の対照から、root cause、破られた不変条件およびfailure oracleまでの因果連鎖を独立に確定する。修正後の一回成功は正常経路の確認に使えるが、root cause推定または原因除去の根拠を兼ねない。固定回数の反復実行や新しい回帰testを一律に要求せず、既存のtest構造で重要な不変条件を安定的、決定的かつ簡潔に保護でき、保守コストに見合う場合だけ追加する。Flakyの再現だけを目的とするsleep、過剰な反復、timing依存、外部障害simulation、またはbuild taskやCI workflowを検査するためだけの新しいtest frameworkは要求しないが、既存の自然なworkflow test harnessによる比例的なtestは妨げない。Build taskやworkflowは、修正前の独立した原因証拠と最も近い原典での修正があれば、修正後の実workflow一回成功で正常経路を確認できる。
