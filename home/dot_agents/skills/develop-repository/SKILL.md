...
- 詳細な調査証拠、検証環境または棄却案を履歴へ残す価値がある場合は、対象commit、目的、内容のoutline、適用範囲および無効化条件を提示し、ユーザーが承認した後だけ `vcs.sh notes add -m MESSAGE [REVISION]` で標準`refs/notes/commits`へ一つのnoteを追加する。`REVISION`はGit revisionまたはexactly one commitへ解決される`jj` revset、bookmarkもしくはchange IDとし、省略時は現在commitを使う。主要な変更理由とuser-visibleな影響はnoteだけに置かずcommit messageを自足させる。既存noteの置換や追記は自動で行わない。
- 明示的にpushを依頼された場合だけ、通常権限で `vcs.sh snapshot --fetch REMOTE` を実行して結果を確認した後、`<develop-repository-dir>/scripts/vcs-push.sh REMOTE BRANCH_OR_BOOKMARK` をsandbox外で実行し、その実行ごとに承認を求める。承認済みnoteもremoteへ反映する場合はbranch/bookmarkとは別に`vcs-push.sh REMOTE --notes`を実行し、両方を別々に確認する。push scriptの再利用可能な承認prefixは要求しない。`jj`でchange bookmarkを生成する場合は第2引数以降を `--change REVISION` とする。
