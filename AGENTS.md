...
`root/` is a separate chezmoi source state for system-level files and is not part of the normal HOME source or bootstrap. When working on a corresponding system file, inspect the live file as the current operational state and update `root/` when the requested desired state should change. For an explicitly requested live apply, use `mise run system:diff -- <absolute-file>`, `mise run system:apply -- <absolute-file>`, and `mise run system:verify -- <absolute-file>`, then confirm the diff is empty. Use only normalized absolute file targets that already exist in `root/`; never use an ad hoc copy or install, pass a directory, or apply all of `/`. Do not introduce a custom-image delivery path without an explicit decision.
